...
func (a *App) updateSidebarActiveView() {
	switch a.currentView {
	case ViewInstances, ViewInstanceDetails, ViewMetadata:
		a.sidebar.SetActiveView(sidebar.ViewInstances)
	case ViewProjectMetadata:
		a.sidebar.SetActiveView(sidebar.ViewProjectMetadata)
...
func (a *App) handleSidebarNavigation(msg sidebar.NavigateMsg) tea.Cmd {
	...
	case sidebar.ViewProjectMetadata:
		// Project metadata requires a project to be selected
		if a.selectedProject == nil {
			return nil
		}
...
func (a *App) renderHeader() string {
	...
	if a.selectedProject != nil {
		...
		if category := a.sidebar.GetCurrentCategory(); category != "" {
			...
			switch a.currentView {
			case ViewInstances, ViewInstanceDetails, ViewMetadata, ViewProjectMetadata, ViewDisks, ViewDiskDetails, ViewSnapshots, ViewSnapshotDetails, ViewImages, ViewImageDetails:
		...
		if a.currentView == ViewProjectMetadata {
			header += a.styles.Muted.Render(" • Project Metadata")
		}
