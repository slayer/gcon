	...
	case tea.KeyMsg:
		...
		if key.Matches(msg, a.keys.Back) {
			...
			case ViewObjects:
				...
				a.updateSidebarActiveView()
				return a, nil
			case ViewInstances, ViewDisks, ViewImages, ViewBuckets, ViewNetworks, ViewFirewall:
...
func (a *App) renderHeader() string {
	...
	if a.selectedProject != nil {
		...
		if category := a.sidebar.GetCurrentCategory(); category != "" {
			...
			switch a.currentView {
			case ViewInstances, ViewInstanceDetails, ViewDisks, ViewDiskDetails, ViewImages, ViewImageDetails:
